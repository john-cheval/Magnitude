    ...
    <section className="text-altermain mt-5 sm:mt-10 lg:mt-20 pb-10 containers">
      {jobList?.map((job, index) => (
        <motion.div
          ...
          key={index + 1}
          className={`flex flex-col md:flex-row md:justify-between gap-x-10 border-t md:items-center border-altermain pt-5 pb-9 ${
            index === jobList.length - 1 ? "border-b " : ""
          }`}
        >
          <div className="space-y-4">
            <h3 className="text-lg sm:text-xl lg:text-2xl leading-[136%]">
              {job?.post_title?.replace(/&amp;/g, "&")}
            </h3>
            ...
            <div
              ...
              className="text-sm space-y-3  text-left font-helvatica font-light !leading-[186%] max-w-[90%]-"
            ></div>
          </div>

          <button
            onClick={handleScroll}
            className="text-sm uppercase px-9 py-4 text-center inline-block bg-altermain text-main w-fit mt-4 md:mt-2 hover:bg-main hover:text-altermain hover:border hover:border-altermain transition-all duration-300 ease-in-out min-w-[200px]"
          >
            Apply Now
          </button>
        </motion.div>
